...
public class Dropdown4 {
	...
	public static void main(String[] args) {
		...
		for(WebElement opt:selectedValues) {
			
			if(opt.getText().equals("Hindi")) {
				val.deselectByVisibleText("Hindi");
				System.out.println("Hindi-- Deselected");
				val.selectByIndex(7);				
			}
		}
		selectedValues = val.getAllSelectedOptions();
		
		for(WebElement value1 : selectedValues) {
			System.out.println(value1.getText());
		}
		
		System.out.println("First Selected Value:"+val.getFirstSelectedOption().getText());
		driver.close();
	}
